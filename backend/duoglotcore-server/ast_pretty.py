...
_additional_child_immediate_nts = {"js": ["js.string", "js.template_chars", "js.template_string", "js.template_substitution"], "py": ["py.string", "py.escape_interpolation", "py.escape_sequence", "py.string_content", "py.interpolation"]}
_additional_self_immediates_before = {"js": ['","', '"("', '")"', '"."'], "py": ['":"', '"("', '")"', '","', '"."', '"["', '"]"']} # exclude immediate nts    # old js: ['"\\""', '"\'"']
_additional_self_immediates_after = {"js": ['"("', '")"','"."'], "py": ['"("', '")"', '"."', '"["', ']"']}
def pretty_mapanno_ast(ast, lang, grammar=None, annotation=None):
  ...
  def _py_hack_init():
    ...
    py_pretty_indent = 0
  def _py_hack_indent(strs):
    nonlocal py_pretty_indent
    py_pretty_indent += 2
    return True
  def _py_hack_dedent(strs):
    nonlocal py_pretty_indent
    py_pretty_indent -= 2
    return True
  ...
  _hack_replacer_dict = {
    ...
    "py": {
      ...
      "EXT._indent": (lambda strs: py_indented_newline[py_pretty_indent], _py_hack_indent),
      "EXT._dedent": (lambda strs:  py_indented_newline[py_pretty_indent], _py_hack_dedent)
    },
  }
  ...
  if lang in _hack_init_dict: _hack_init_dict[lang]()
  immediate_after_flag = False

  # ranges_by_ntid = {}
  def _pretty_ast_rec(ast):
