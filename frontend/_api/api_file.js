...
async function deleteMultipleFilesAsync(limiting_folder, filepaths) {
  ...
  let resp_str = await postAsync(SERVER_FILE_PREFIX, "/removemultiplefile", {limiting_folder: limiting_folder, filepaths:filepaths});
  let resp = JSON.parse(resp_str);
  return resp;
}
...
async function listDirAllMetaLocalAsync(dirpath, include_pattern, ignore_pattern) {
  let resp_str = await postAsync(SERVER_FILE_PREFIX, "/listdirallmeta", {dirpath:dirpath, include_pattern:include_pattern, ignore_pattern:ignore_pattern});
  return resp = JSON.parse(resp_str);
}

async function listDirAllMetaRemoteAsync(remote_origin, dirpath, include_pattern, ignore_pattern) {
  let resp_str = await postAsync(remote_origin, "/listdirallmeta", {dirpath:dirpath, include_pattern:include_pattern, ignore_pattern:ignore_pattern});
  return resp = JSON.parse(resp_str);
}
