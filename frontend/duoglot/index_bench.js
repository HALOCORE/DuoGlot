...
let task_queue = [];
function task_queue_label_update() {statusQueueSetter("Task Queue Status\nLength: " + task_queue.length + "\nAddAll: " + addall_benchmark_ids.length);}
function task_queue_pushtail(benchmark_id) { task_queue.push(benchmark_id); task_queue_label_update(); }
function task_queue_pushtail_list(benchmark_id_list) { for (let benchmark_id of benchmark_id_list) {task_queue.push(benchmark_id);} task_queue_label_update(); }
function task_queue_pophead() { if (task_queue.length > 0) {let head = task_queue.shift(); task_queue_label_update(); return head;} else return null; }
function task_queue_clear() { task_queue.length = 0; task_queue_label_update(); }
...
setTimeout((async function main1() {
  ...
  task_runners[0].logger = composeFuncN(dashboard_iframe_elem.contentWindow.createLogger("logSub1"), (msg) => task_runners[0].log_string_list.push(msg));
  task_runners[0].logger_cleaner = dashboard_iframe_elem.contentWindow.createLoggerCleaner("logSub1");
  task_runners[1].logger = composeFuncN(dashboard_iframe_elem.contentWindow.createLogger("logSub2"), (msg) => task_runners[1].log_string_list.push(msg));
...
async function initializeConfigAsync() {
  ...
  async function _updateConfigAsync() {
    ...
    let stafiles = sourceLang in file_dict["dirs"] ? file_dict["dirs"][sourceLang]["files"] : []; //TODO: get stafiles according to source language
...
async function runBenchmarkHandler(benchmarkname) {
  ...
  let benchDirname = benchfolder_input_elem.value;
  let sourcelang = sourcelang_select_elem.value;
  let targetlang = targetlang_select_elem.value;
  let sourceProgPrefix = transprog_source_select_elem.value;
  let transProgPrefix = transprog_trans_select_elem.value;
  let transtestProgPrefix = transprog_transtest_select_elem.value;
  let targetProgPrefix = transprog_target_select_elem.value;
  let srckey = sourcelang + "_" + sourcelang;
  let tarkey = targetlang + "_" + targetlang;
  let pairkey = sourcelang + "_" + targetlang;
  let staFilepath = "duoglot/tests/" + benchDirname + "/" + sourcelang + "/" + benchmarkname;
  let sourceProgPath = sourceProgPrefix === "" ? null : "duoglot/tests/trans_programs/" + srckey + "/" + sourceProgPrefix + ".snart";
  let transProgPath = transProgPrefix === "" ? null : "duoglot/tests/trans_programs/" + pairkey + "/" + transProgPrefix + ".snart";
  let transProgConfigPath = transProgPrefix === "" ? null : "duoglot/tests/trans_programs/" + pairkey + "/" + transProgPrefix + ".config.json";
  let transtestProgPath = transtestProgPrefix === "" ? null : "duoglot/tests/trans_programs/" + pairkey + "/" + transtestProgPrefix + ".snart";
  let targetProgPath = targetProgPrefix === "" ? null : "duoglot/tests/trans_programs/" + tarkey + "/" + targetProgPrefix + ".snart";
  ...
  try {
    ...
    if (source_prog_code !== null) {
      ...
      testcall_code = raw_testcall_code;
