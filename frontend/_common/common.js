...
function error404Nullify(err) {
  if (String(err) === "NOT FOUND") return null;
  throw err;
}
...
function parse_gitstatus(content) {
  ...
  if (lines[0] !== "GITSTATUS") throw Error("Unexpected gitstatus content.");
...
function collect_inc_until(array, start_idx, until_func) {
  ...
  for (; result_idx < array.length; result_idx++) {
    if (until_func(array[result_idx])) result.push(array[result_idx]);
    else break;
  }
...
function monacoApplyDecorations(code_editor, decos) {
  ...
  code_editor.__my_old_deco_ids = code_editor.deltaDecorations(
    code_editor.__my_old_deco_ids ? code_editor.__my_old_deco_ids : [], decos);
}

function monacoApplyGroupDecorations(code_editor, decos, group_name) {
  //see the doc about adding decorations in monaco editor
  //https://microsoft.github.io/monaco-editor/playground.html#interacting-with-the-editor-line-and-inline-decorations
  if (code_editor.__my_old_deco_groups_ids === undefined || code_editor.__my_old_deco_groups_ids === null) code_editor.__my_old_deco_groups_ids = {}; 
  code_editor.__my_old_deco_groups_ids[group_name] = code_editor.deltaDecorations(
    code_editor.__my_old_deco_groups_ids[group_name] ? code_editor.__my_old_deco_groups_ids[group_name] : [], decos);
}
